...
def get_deterministic_probs(image_bytes, num_classes):
    """Generates consistent probabilities based on image content for Demo Mode."""
    import hashlib
    h = hashlib.md5(image_bytes).hexdigest()
    seed = int(h, 16) % (2**32)
    rs = np.random.RandomState(seed)
    probs = rs.dirichlet(np.ones(num_classes)).tolist()
    return probs
...
def load_eye_model():
    ...
    try:
        ...
        MODEL_PATH  = os.environ.get("EYE_MODEL_PATH",  os.path.join(BASE_DIR, "neoscan_eye_FINAL_effb4.keras"))
        CONFIG_PATH = os.environ.get("EYE_CONFIG_PATH", os.path.join(BASE_DIR, "eye_model_config_FINAL.json"))
        if os.path.exists(MODEL_PATH):
            EYE_MODEL = tf.keras.models.load_model(MODEL_PATH)
            print(f"[OK] Eye model loaded from {MODEL_PATH}")
        else:
            print(f"[WARN] Eye model not found at {MODEL_PATH} — running in DEMO mode")
        if os.path.exists(CONFIG_PATH):
            with open(CONFIG_PATH) as f:
                EYE_CONFIG = json.load(f)
            print(f"[OK] Eye config loaded")
        else:
            EYE_CONFIG = {
                "idx_to_class": {
        ...
        print(f"[WARN] Eye model load error: {e} — running in DEMO mode")
...
def load_skin_model():
    ...
    try:
        ...
        MODEL_PATH  = os.environ.get("SKIN_MODEL_PATH", os.path.join(BASE_DIR, "best_model.pth"))
        ...
        model.eval()
        SKIN_MODEL = model
        print(f"[OK] Skin model loaded from {MODEL_PATH}  (epoch {ckpt.get('epoch','?')}, val_acc={ckpt.get('val_acc',0):.4f})")
    except Exception as e:
        print(f"[WARN] Skin model load error: {e} — running in DEMO mode")
...
def serve_index():
    ...
    return {"status": "NeoScan AI API running", "error": "Frontend not found"}
...
async def predict_eye(
    ...
    if EYE_MODEL is None:
        ...
        probs   = get_deterministic_probs(image_bytes, 6)
...
async def predict_skin(file: UploadFile = File(...)):
    ...
    if SKIN_MODEL is None:
        ...
        probs   = get_deterministic_probs(image_bytes, 4)
        pred_idx = int(np.argmax(probs))
        pred    = classes[pred_idx]
        risk    = SKIN_RISK[pred]
        return {
...
def load_body_model():
    ...
    try:
        ...
        MODEL_PATH  = os.environ.get("BODY_MODEL_PATH", os.path.join(BASE_DIR, "body_model.pth"))

        if not os.path.exists(MODEL_PATH):
            print(f"[WARN] Body model not found at {MODEL_PATH} — running in DEMO mode")
            return
        ...
        model.eval()
        BODY_MODEL = model
        print(f"[OK] Body model loaded from {MODEL_PATH}")
    except Exception as e:
        print(f"[WARN] Body model load error: {e} — running in DEMO mode")
...
async def predict_body(file: UploadFile = File(...)):
    ...
    if BODY_MODEL is None:
        ...
        probs   = get_deterministic_probs(image_bytes, 2)
        pred_idx = int(np.argmax(probs))
        pred    = classes[pred_idx]
        risk    = BODY_RISK[pred]
        return {
...
def load_face_models():
    ...
    try:
        ...
        vgg_path = os.environ.get('FACE_VGG_PATH', os.path.join(BASE_DIR, 'VGG16_best.pth'))
        if os.path.exists(vgg_path):
            ...
            vgg.eval()
            FACE_VGG = vgg
            print(f'[OK] Face VGG16 loaded from {vgg_path}')
        else:
            print(f'[WARN] Face VGG16 not found at {vgg_path}')
        ...
        try:
            ...
            vit_path = os.environ.get('FACE_VIT_PATH', os.path.join(BASE_DIR, 'ViT_v2_best.pth'))
            if os.path.exists(vit_path):
                ...
                vit.eval()
                FACE_VIT = vit
                print(f'[OK] Face ViT loaded from {vit_path}')
            else:
                print(f'[WARN] Face ViT not found at {vit_path}')
        except ImportError:
            print('[WARN] timm not installed -- ViT not loaded. Run: pip install timm')

    except Exception as e:
        print(f'[WARN] Face model load error: {e} -- running in DEMO mode')
...
async def predict_face(file: UploadFile = File(...)):
    ...
    if FACE_VGG is None and FACE_VIT is None:
        classes = FACE_CLASS_NAMES
        probs   = get_deterministic_probs(image_bytes, 3)
        pred_idx = int(np.argmax(probs))
        pred    = classes[pred_idx]
        risk    = FACE_RISK[pred]
        return {
            ...
            'risk_color'   : risk['color'],
            'advice'       : risk['advice'],
            'probabilities': {FACE_LABELS[c]: round(p * 100, 2) for c, p in zip(classes, probs)},
            'mode'         : 'demo',
            'model_used'   : 'demo'
        }
...
# ── MOUNT STATIC FILES ───────────────────────────────────────
# Mount the entire Frontend directory to serve CSS, JS, Images
